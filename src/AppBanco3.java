...
import banco.ContaGeral;

public class AppBanco3 {
    private static ContaGeral[] contas = 
        { new ContaBancaria("Ana", 1000.00),
          new ContaEspecial("Bob", 800.00, 2000),
          new ContaEspecial("Charles", 450.00, 2000) };
        ...
        private static void opcaoAumentarLim() {
            double v = inputDouble("Valor a ser adicionado ao limite de todas: ");
            for(int i = 0; i < contas.length; i++){
                contas[i].alterarLimite(v + contas[i].consultarLimite());
            }
            print("\n## Limite Aumentado");
        }
}
